				...
				if(vipPress && goldPress) {
					if(e.target.dataset.plan === 'anual') {
						...
						if(document.body.classList.contains('dashboard-body')) {
							goldTime.innerHTML = '/ Año <span style="font-size: .7em;">(Obtienes 2 meses gratis)</span>';
							vipTime.innerHTML = '/ Año <span style="font-size: .7em;">(Obtienes 2 meses gratis)</span>';
						}else {
...
(() => {
	...
	const headerToggleElements = document.getElementById('header-toggle-elements');

	if(headerToggleElements) {
		document.addEventListener('click', e => {
			if( e.target !== headerToggleElements &&
...
(() => {
	...
	if(formSteps) {
		// Variables
		const formStepsContainer = document.querySelectorAll('.form-control-steps__container');
		...
		function changeStep(e) {
			if(e.target.classList.contains('form-steps__btn')) {
				...
				let currentNavItem = stepsNav.querySelector(`[data-step-target="${e.target.dataset.stepPage}"]`);

				// Next
				let nextStep = ++currentStep;
				let nextStepSection = formSteps.querySelector(`[data-step-page="${nextStep}"]`);
				let nextNavItem = stepsNav.querySelector(`[data-step-target="${nextStep}"]`);
		...
		function validateSupport(button) {
			const itemsActive = Array.from(document.querySelectorAll('.support-nav__item.active'));
			const sectionsActive = itemsActive.map(item => document.querySelector(item.getAttribute('href')));

			const arrResultG = sectionsActive.map(section => {
				if(section) {
					if(section.querySelector('[type="checkbox"]:not(.optional):checked')) {
		...
		function beforeStep(e) {
			...
			let currentStepSection = formSteps.querySelector(`section[data-step-page="${currentStep}"]`);
			let currentNavItem = stepsNav.querySelector(`[data-step-target="${currentStep}"]`);
			
			// Solución para rastro
			currentNavItem.classList.remove('check');
			if(currentStep == document.querySelector('[data-step-desc="Confirma"]').dataset.stepPage) {
				currentNavItem.parentElement.classList.remove('color-bar');
			}else {
				currentNavItem.parentElement.previousElementSibling.classList.remove('color-bar');
			}
			currentNavItem.innerHTML = currentNavItem.dataset.stepTarget;

			// Prev
			let prevStep = --currentStep;
			let prevStepSection = formSteps.querySelector(`[data-step-page="${prevStep}"]`);
			let prevNavItem = stepsNav.querySelector(`[data-step-target="${prevStep}"]`);
		...
		function setPaddingToInputs() {
			if(formStepsContainer.length > 0) {
				formStepsContainer.forEach(container => {
					...
					const placeholder = container.querySelector('.form-control-steps__placeholder');
					if(placeholder) {
						const padding = placeholder.getBoundingClientRect().width;

						container.querySelector('.form-control-steps').style.paddingLeft = `calc(2em + ${padding}px)`;
					}
		...
		function configNavDesc() {
			...
			const currentNavItem = stepsNav.querySelector(`[data-step-target="${currentStep}"]`);
			const currentStepSection = formSteps.querySelector(`section[data-step-page="${currentStep}"]`);
...
(() => {
	...
	const btnSupport = document.querySelector('[data-step-desc="Pasos"] button[data-step-page]');
	...
	if(supportNav) {
		...
		function activeSupport(e) {
			...
			if(e.target.classList.contains('support-nav__item')) {
				target = e.target;
			}else if(e.target.parentElement.classList.contains('support-nav__item')) {
			...
			if(target) {		
				if(target.classList.contains('active')){
					removePrevShow();	
					document.querySelector(target.getAttribute('href')).classList.add('s-show');
				}else {
					if(canNext()) {
				...
				const removeSection = document.querySelector(e.target.parentElement.parentElement.getAttribute('href')) 

				removeSection.classList.remove('s-show');

				removeSection.querySelectorAll('[type="checkbox"]:checked').forEach(checkbox => {
					checkbox.checked = false;
				});

				removeSection.querySelectorAll('input:not([type="checkbox"])').forEach(input => {
					input.value = '';
				});
			}
		}
	...
	function canNext() {
		...
		if(sectionActive) {
			if(sectionActive.querySelector('[type="checkbox"]:not(.optional):checked')) {
				const inputs = sectionActive.querySelectorAll('input:not([type="checkbox"])');
				if(inputs.length > 0) {
					const arrayResult = [];
					inputs.forEach(input => {
						if(input.value !== '' && input.validity.valid) {
...
(() => {
	...
	// input step 1
	new validateInput(['#url-demo'], document.querySelector('[data-step-desc="Fuente"] button[data-step-page]'));
	new validateInput(['#url-track'], document.querySelector('[data-step-desc="Subir"] button[data-step-page]'));
	// input step 4
	if(document.querySelector('#music-price')) {
		new validateInput(['#artist-name', '#music-title', '#music-desc', '#music-price'], document.querySelector('[data-step-desc="Título"] button[data-step-page]'));
	}else {
		new validateInput(['#artist-name', '#music-title', '#music-desc'], document.querySelector('[data-step-desc="Título"] button[data-step-page]'));
	}
	...
	if(gender && otherGender) {
		const btnGender = gender.parentElement.parentElement.parentElement.querySelector('button[data-step-page]');
	...
	const buttonGateLink = document.querySelector('[data-step-desc="Privacidad"] button[data-step-page]');
	...
	if(gateLink) {
		gateLink.addEventListener('input', e => {
			if(gateLink.value !== ''
				...
				gateLinkOutput.querySelector('span').innerHTML = gateLink.value.toLowerCase();
			}else {
...
(() => {
	...
	const btnStepDesing = document.querySelector('[data-step-desc="Diseño"] button[data-step-page]');
	const confirmationCardBannerImg = document.querySelector('.confirmation-card__banner img')
	...
	if(desingSocial) {
		...
		function toggleDesingSocial() {
			const socialNav = desingPreview.querySelector('.main-gate__footer > .row > *:first-child');
		...
		function uploadingProccess(file, event) {
			...
			if(maxSizeFile !== 'unlimited') {
				if(size > Number(maxSizeFile)) {
					...
					infoDrop.innerHTML = `No puedes subir archivos mayores a ${maxSizeFile === '10' ? '10mb' : '1gb'} actualiza tu membresía<br><label for="music-file">Sube otro archivo</label> o arratralo aquí`;
					return;
				}
			}

			if(musicFile.getAttribute('accept') === 'audio/*, .zip, .rar') {
				if(file.type !== 'audio/mpeg' && file.type !== 'audio/mp3' && file.type !== 'audio/wav' && file.type !== 'audio/aiff' && file.type !== 'application/zip') {
					dropArea.classList.add('drop-error', 'text-center');
					iconState.innerHTML = '<i class="fa fa-times"></i>';
...
(() => {
	...
	const btnSubmit = formUpdateProfile.querySelector('[type="submit"]');
		...
		inputs.forEach(input => {
			...
			}else if(input.getAttribute('type') === 'text' || input.getAttribute('type') === 'password' || input.getAttribute('type') === 'email') {
	...
	const btnSubmit = formUpdateProfile.querySelector('[type="submit"]');
		...
		btnSubmit.addEventListener('click', e => {
			...
			for (var i=0; i<formUpdateProfile.length; i++) {
                ...
                if(formUpdateProfile[i].type =='text' || formUpdateProfile[i].type =='email') {
							if (formUpdateProfile[i].name == 'tiktok' || formUpdateProfile[i].name == 'twitter' ||formUpdateProfile[i].name == 'spotify' ||formUpdateProfile[i].name == 'souncloud' || formUpdateProfile[i].name == 'mixcloud' ||formUpdateProfile[i].name == 'youtube' ||formUpdateProfile[i].name == 'facebook'||formUpdateProfile[i].name == 'instagram'){
						
					   todoCorrecto=true;

					   }else   if (formUpdateProfile[i].value == null || formUpdateProfile[i].value.length == 0 || /^\s*$/.test(formUpdateProfile[i].value)){
								Swal.fire({
									icon: 'warning',
									text: formUpdateProfile[i].name+ ' no puede estar vacío o contener sólo espacios en blanco',
									button: true
								});
		...
		if (todoCorrecto ==true) {formUpdateProfile.submit(); console.log("hola")}
				...
				if(e.target.classList.contains('add-other__btn')) {
					if(!document.querySelector('[data-step-desc="Pasos"].basic')) {
						if(!document.querySelector('[data-step-desc="Pasos"].gold')) {
							if(otherContainer.querySelectorAll('.remove-other__btn').length < 4) {
								...
								const other = `
									<div class="add-other mt-3">
										<input type="url" class="form-control-steps" name="" placeholder="${oInput.getAttribute('placeholder')}">
								...
								Swal.fire({
									icon: 'warning',
									title: 'Solo se pueden agregar campos ilimitados en suscripción Gold',
									text: 'Te invitamos a mejorar tu membresía',
									button: false
								})
							}
